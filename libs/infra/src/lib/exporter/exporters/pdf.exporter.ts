...
export class PdfExporter extends Exporter {
  ...
  addPassportTable(document: typeof PDFKit) {
    ...
    table.addBody([
      ...
      {employer: 'Зарегистрирован по адресу: г. Москва, улица свободы, дом 56', landlord: 'Зарегистрирован по адресу:  г. Москва, улица свободы, дом 56'},
  ...
  async createDocumentFromContract(contract: Domain.Contract, language: Language = Language.RU): Promise<Stream> {
    ...
    this.writeLine(document, `2.5. Наймодатель оплачивает: <Доработать>`)
    ...
    this.writeLine(document, `5.5. В случае досрочного расторжения договора найма Нанимателем производится пересчет оплаты за период найма в соответствие со следующими условиями: <ДОРАБОТАТЬ>.`)
    this.writeLine(document, `5.6. Залоговая сумма возвращается Нанимателю при <ОБСУДИТЬ И ДОРАБОТАТЬ>`)
