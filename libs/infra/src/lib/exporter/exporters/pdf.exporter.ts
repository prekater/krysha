...
export class PdfExporter extends Exporter {
  ...
  async createDocumentFromContract(contract: Domain.Contract, language: Language = Language.RU): Promise<Stream> {
    ...
    this.writeLine(document, `5.7. Договор аренды может быть расторгнут по требованию Арендатора в следующих случаях:`)
    this.writeLine(document, '5.7.1. Помещение в силу обстоятельств, за которые Арендатор не отвечает, окажется в состоянии, непригодном для использования.', {indent: PdfExporter.INDENT})
    this.writeLine(document, '5.7.2. Арендодатель не предоставляет Помещение в пользование Арендатору либо создает препятствия пользованию в соответствии с условиями Договора или назначением Помещения.', {indent: PdfExporter.INDENT})
    this.writeLine(document, '5.7.3. Помещение имеет препятствующие пользованию им недостатки, которые не были оговорены Арендодателем при заключении Договора, не были заранее известны Арендатору и не должны были быть обнаружены Арендатором во время осмотра при заключении Договора.', {indent: PdfExporter.INDENT})

    this.writeLine(document, `5.8. Стороны берут на себя обязательства принимать все меры к разрешению разногласий путем переговоров до полного урегулирования предмета разногласий.`)
    this.writeLine(document, `5.9. В случае невозможности достигнуть согласия путем переговоров Стороны разрешают разногласия путем обращения в суд общей юрисдикции в установленном законом порядке.`)
